...
public class SimpleEnemy : MonoBehaviour
{
	...
	public bool CanISeePlayer ()
	{
		...
		var inFOV = Vector3.Angle (transform.forward, playerDirection) < 45; //90 degree field of fiew
		if (inFOV) {	
			RaycastHit hit;
	...
	{
		var destination = transform.position + new Vector3 (Random.Range (-20, 20), 0, Random.Range (-20, 20));
    ...
    public Action CycTest()
    {
        yield return NodeResult.Success;
    }
    public IEnumerator<NodeResult> ActionCycTest()
    {
        yield return NodeResult.Success;	
    }
